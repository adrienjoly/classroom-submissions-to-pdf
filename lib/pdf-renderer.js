...
function renderHtmlFile(htmlLines) {
  return [].concat([
    '<!DOCTYPE html>',
    '<html>',
    '  <head>',
...
const renderCodeToHtml = (code) => renderHtmlFile(renderStudentHtml(code))

const renderStudentsToHtml = (students) => renderHtmlFile(students.map(renderStudentHtml))
...
module.exports = {
  renderCodeToHtml: renderCodeToHtml, // one student's code -> one html file
  renderHtml: renderStudentsToHtml, // many student submissions -> one html file
  genPdfFileFromHtml: genPdfFileFromHtml, // one html file -> one pdf file
}
